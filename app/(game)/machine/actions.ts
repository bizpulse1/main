"use server";

import { redirect } from "next/navigation";
import { createClient } from "@/lib/supabase/server";
import { getOrCreateCurrentTurn } from "@/lib/game/turns";

export async function purchaseMachine(formData: FormData) {
  const supabase = createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  if (!user) redirect("/");

  const companyId = formData.get("company_id") as string | null;
  if (!companyId) redirect("/hub");

  const { data: company } = await supabase
    .from("companies")
    .select("*")
    .eq("id", companyId)
    .eq("owner_user_id", user.id)
    .single();
  if (!company) redirect("/hub");
  if (company.activity_type !== "industrial") redirect(`/turn?company=${companyId}`);

  // Already own a machine — one is enough for now (assembly lines /
  // multiple machines are a later scope, not built).
  const { data: existing } = await supabase
    .from("company_machines")
    .select("id")
    .eq("company_id", companyId)
    .eq("status", "active")
    .maybeSingle();
  if (existing) redirect(`/turn?company=${companyId}`);

  const { data: machine } = await supabase.from("machines_catalog").select("*").limit(1).single();
  if (!machine) throw new Error("No machine configured — check seed data");

  if (machine.price > company.capital) {
    redirect(`/machine?company=${companyId}&error=insufficient_funds`);
  }

  const turn = await getOrCreateCurrentTurn(supabase, company.session_id);

  const { data: purchased, error: purchaseError } = await supabase
    .from("company_machines")
    .insert({
      company_id: companyId,
      machine_catalog_id: machine.id,
      purchased_turn_id: turn.id,
      financed_via: "own_funds",
      wear_pct: 0,
      cumulative_production: 0,
      status: "active",
    })
    .select()
    .single();
  if (purchaseError || !purchased) {
    throw new Error(purchaseError?.message ?? "Failed to purchase machine");
  }

  const { error: ledgerError } = await supabase.from("treasury_ledger").insert({
    company_id: companyId,
    turn_id: turn.id,
    movement_type: "machine_purchase",
    direction: "out",
    amount: machine.price,
    reference_table: "company_machines",
    reference_id: purchased.id,
  });
  if (ledgerError) throw new Error(ledgerError.message);

  const { error: capitalError } = await supabase
    .from("companies")
    .update({ capital: company.capital - machine.price })
    .eq("id", companyId);
  if (capitalError) throw new Error(capitalError.message);

  redirect(`/turn?company=${companyId}`);
}
